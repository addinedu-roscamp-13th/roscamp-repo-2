"""Robot-side human-follow control loop.

The browser only starts/stops this loop. The robot backend owns camera polling,
AI inference requests, and motor commands, like the parking/docking loops.
"""
...
from app.routers.drive import _motor_send
from app.security import create_access_token

router = APIRouter(prefix="/api/robot/human-follow", tags=["human-follow-robot"])


class HumanFollowConfig(BaseModel):
    ai_server_url: str = "http://192.168.1.4:9001"
    model: str = "yolov8n"
    ...
    max_duration_s: float = Field(30.0, ge=0.0, le=600.0)  # 0 = 무제한, 초과 시 자동 종료
...
def _analyze_jpeg(ai_server_url: str, jpeg: bytes, model: str, confidence: float) -> dict[str, Any] | None:
    ...
    add_field("confidence", str(confidence))
    parts.append(f"--{boundary}--\r\n".encode())
...
def _best_target(detections: list[dict[str, Any]], labels: set[str]) -> dict[str, Any] | None:
    ...
    if not candidates:
        return None
    return max(candidates, key=lambda d: float(d.get("confidence") or 0.0))
...
async def _follow_loop(cfg: HumanFollowConfig) -> None:
    ...
    try:
        ...
        while True:
            if cfg.max_duration_s > 0 and time.monotonic() - started >= cfg.max_duration_s:
                await _motor_send(0, 0)
                _state.update(running=False, phase="idle", message=f"최대 지속시간({cfg.max_duration_s:g}s) 종료", telemetry={})
                return
            ...
            if not jpeg:
                ...
                continue
            try:
                result = await asyncio.to_thread(_analyze_jpeg, cfg.ai_server_url, jpeg, cfg.model, cfg.confidence)
                ...
                _state.update(
                    ...
                    telemetry={
                        ...
                        **{k: round(float(v), 3) for k, v in tele.items()},
                    },
                    last_error=None,
                )
...
async def status():
    ...
    if _task is not None:
        task = {"done": _task.done(), "cancelled": _task.cancelled()}
