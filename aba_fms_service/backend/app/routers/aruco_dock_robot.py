...
class DockConfig(BaseModel):
    ...
    lpark_x_tol_m: float = Field(0.010, ge=0.002, le=0.08)      # 카메라 중심선-마커 중심 좌우 허용 오차
    lpark_yaw_tol_deg: float = Field(8.0, ge=2.0, le=30.0)     # 완료/전진 허용 yaw 오차
    ...
    try:
        ...
        while True:
            ...
            if cfg.pose_center_enable and p is not None:
                ...
                x_abs = abs(p["x_m"])
                yaw_abs = abs(p["yaw_deg"])
                center_ready = bearing_abs <= bearing_tol and x_abs <= cfg.lpark_x_tol_m and yaw_abs <= cfg.lpark_yaw_tol_deg
                if cfg.use_wall_sensor and wall_cm is not None and wall_cm <= cfg.target_wall_cm and center_ready:
                    await _complete_parking(
                        cfg,
                        f"벽 앞 주차 완료 — {round(wall_cm, 1)}cm",
                        {"wall_cm": wall_cm, "x_m": round(p["x_m"], 3), "z_m": round(p["z_m"], 3), "yaw_deg": round(p["yaw_deg"], 1)},
                    )
                    break
                if not center_ready:
                    linear = 0.0
                ...
                _state["telemetry"] = {
                    ...
                    "yaw_tol_deg": cfg.lpark_yaw_tol_deg,
