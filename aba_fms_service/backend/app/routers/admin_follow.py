...
async def release_admin_follow(req: AdminFollowRequest) -> AdminFollowReleaseResponse:
    """추종 종료 보고. 로봇 상태를 검사하지 않는다 — 종료는 언제나 받아줘야 한다.

    승인받은 적 없는 로봇이 보내도 에러가 아니다(released=False). GUI 는 FMS 응답과
    무관하게 로컬 추종을 멈추므로, 여기서 막아봐야 양쪽 기록만 더 어긋난다.
    """
    with _grants_lock:
