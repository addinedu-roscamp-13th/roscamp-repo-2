...
/// 이 정점에서 **실제로 서야 하나.** 서야 하면 선행통과를 안 준다(정확히 닿아야 한다).
///
/// ⚠️ [2026-08-07] **순회에는 서는 정점이 없다.** 예전엔 `idx + 1 >= path.size()` 하나로
///    판정해서, 랩의 마지막 정점이 배달의 최종 목적지와 같은 취급을 받았다. 거기서만
///    선행통과가 꺼져(`reach = arrive_radius`, 0.05m) 로봇이 **완전히 감속·정지**한 뒤
///    새 목표를 받고 다시 가속했다 — 랩을 한 바퀴 돌 때마다 한 번씩 주춤한다.
///    사용자 보고 2026-08-07: "순회가 끝나면 좀 빨리 경로를 받았으면".
///
///    랩은 끝나지 않는다. 마지막 정점도 **경유점**이므로 다른 정점과 같이 다뤄야 한다.
///    (실제 정지는 배달의 최종 정점·도킹에서만 필요하다 — 정밀 정지·완료 판정 때문에.)
///
/// ⚠️ 순회에서 선행통과가 켜지면 도착 판정이 정점 **반경 밖**에서 난다. 그 순간 랩을
///    재생성하는 쪽이 `nearest(현재 좌표)` 를 쓰면 **직전 정점**이 뽑혀 랩이 되감긴다 —
///    그래서 호출부는 재생성 기준점을 **방금 지난 정점으로 명시**해야 한다
///    (`make_patrol_path` 의 snap 인자).
inline bool is_stop_node(bool patrol, std::size_t idx, std::size_t path_size)
{
