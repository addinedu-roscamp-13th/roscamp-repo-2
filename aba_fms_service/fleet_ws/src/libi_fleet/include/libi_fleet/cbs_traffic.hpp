...
public:
  ...
  : tick_seconds_(env_d("LIBI_CBS_TICK_SEC", 1.0)),
    ...
    // 예약 시각의 허용 오차는 **비대칭이다 — 일찍 10초, 늦게 3초**(tick_seconds_=1.0 기준).
    //   slack_       = 일찍 온 쪽 — 계획보다 이만큼 빠르면 그냥 통과시킨다
    //   drift_limit_ = 늦게 온 쪽 — 예약 시각을 **이만큼 넘기면** 그 자리에서 컷이다
    //
    // ## 왜 3 인가 (사용자 스펙 2026-08-02)
    //
    // 스펙은 "지연시간이 넘어가면 **바로** 재계획" 이다. 그 '지연시간' 이 이 값이고,
    // 화면은 여기까지 남은 초를 카운트다운해 0 이 되는 순간 재계획이 걸린다.
    //
    // 하한은 **1** 이다. 틱이 1초라 0 으로 두면 0.1초 흔들림도 틱 경계를 넘는 순간
    // 지연이 되어 재계획이 폭주한다(실측 2026-08-02: 2분에 16회, 전부 "초과 0.1s").
    // 실측 오차가 0.1~0.3초이므로 3 이면 10배 여유이고, 진짜 막힘은 3초에 잡는다.
    //
    // ⚠️ **관용은 계획이 넉넉할 때만 작아질 수 있다.** 계획이 실물보다 빠르면 로봇은
    //    늘 늦고, 관용을 아무리 줄여도 재계획만 늘 뿐 아무것도 안 고쳐진다. 그래서
    //    `node_stop_ticks_` 를 3 으로 올려 간선마다 여유를 실었다 — 그쪽 주석 참고.
    //
    ...
    drift_limit_(env_i("LIBI_CBS_DRIFT_LIMIT", 3)),
    // 간선마다 무조건 더하는 여유(틱). 노드 진입 감속·정지·재출발에 실제로 드는 시간이자,
    // **관용을 작게 유지하기 위한 밑천**이다.
    //
    // ⚠️ [2026-08-02] 1 → 3. 실측에서 계획 17초짜리 칸을 로봇이 17.3초에 지났다 —
    //    계획이 실물보다 **2% 빠르다.** 관용(`drift_limit_`)을 3 으로 조인 상태에서
    //    그대로 두면 평상시에도 마감이 아슬아슬하다. 여기서 2초를 더 얹으면 로봇이
    //    평소엔 **일찍** 도착하고(그건 `slack_` 이 통과시킨다), 마감은 **진짜 막혔을 때만**
    //    걸린다. 관용을 줄이는 값이 아니라 관용을 줄일 수 있게 해 주는 값이다.
    //
    //    ⚠️ 대가: 계획 소요가 간선당 2초씩 늘어 CBS 가 로봇을 더 벌려 놓는다(처리량 감소).
    //       재계획이 충분히 드물어지면 다시 낮춰 본다. 실측이 유일한 근거다.
    node_stop_ticks_(env_i("LIBI_CBS_NODE_STOP_TICKS", 3))
  {
  }
