...
public:
  ...
  : tick_seconds_(env_d("LIBI_CBS_TICK_SEC", 1.0)),
    ...
    // 예약 시각의 허용 오차는 **비대칭이다 — 일찍 10초, 늦게 0초**(tick_seconds_=1.0 기준).
    //   slack_       = 일찍 온 쪽 — 계획보다 이만큼 빠르면 그냥 통과시킨다
    //   drift_limit_ = 늦게 온 쪽 — **관용 없음.** 예약 시각을 넘긴 순간 시간표를 버린다
    //
    // ## 왜 지연 쪽만 0인가 — 두 방향은 대칭이 아니다
    // 계획은 "이 정점을 이 시각에 비운다"를 전제로 **남에게 통과를 열어 줬다.** 늦은
    // 순간 그 전제가 깨지고, 시간표는 이미 틀린 허가를 내주고 있는 상태다. 봐주는 폭은
    // 그 틀린 허가를 그만큼 더 오래 유지하겠다는 뜻이다 — 그래서 0이다.
    ...
    drift_limit_(env_i("LIBI_CBS_DRIFT_LIMIT", 0)),
