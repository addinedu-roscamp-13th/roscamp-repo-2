...
struct PlanOptions
{
  ...
  int max_nodes{4000};
  // 로봇별 **출발 시각**(틱). 비어 있으면 전부 0(예전과 같다).
  //
  // ⚠️ [2026-08-02] 이게 없으면 **이동 중 재계획이 구조적으로 늦는다.**
  //    로봇이 간선을 타고 있는데 재계획하면 계획은 그 목적지 정점을 `start` 로 잡는다.
  //    그런데 로봇은 아직 **거기 없다** — 도착까지 남은 시간이 있다. 0 으로 시작하면
  //    계획은 "이미 도착했다" 고 보고 짜므로, 실제로 도착해 다음 간선을 요청하는 순간
  //    `now - depart` 가 그 이동 시간만큼 벌어져 **무조건 지연 강등**이 난다.
  //    (실기 실측 2026-08-02: 재계획 사유의 90%가 그것 — 6~7초에 한 번씩 시간표를 버렸다)
  //
  //    그래서 커밋 간선의 예산을 **출발 시각으로** 넣는다. 그러면 arrive/depart 뿐 아니라
  //    **충돌 제약까지 같은 시간축**에서 밀린다. 결과 배열만 사후에 offset 하면 다른
  //    로봇과의 충돌 보장이 깨진다(codex 지적 P0).
