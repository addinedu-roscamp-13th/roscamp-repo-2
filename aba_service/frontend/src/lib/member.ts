 ...
 * 열람 요청 시 고를 수 있는 자리.
 *
 * 실제 로봇 지도 정점은 테이블당 3개(상/좌/우 또는 하/좌/우)씩 있지만, 회원이
 * 고를 땐 "테이블 1번"/"테이블 2번" 두 가지만 보여준다 — 사용자는 어느
 * 정점인지 몰라도 되고 몰라야 자연스럽다. 각 테이블의 대표 정점 하나만 쓴다
 * (ponytail: 대표 정점은 임의로 상/하를 골랐다 — 실제 배치상 더 적절한
 * 정점이 있으면 값만 바꾸면 된다. 백엔드 화이트리스트는 6개 정점을 그대로
 * 허용하므로 여기서 값을 바꿔도 백엔드 쪽은 손댈 필요 없다).
 */
export const TABLES: { value: string; label: string }[] = [
  { value: "테이블-1번-상", label: "테이블 1번" },
  { value: "테이블-2번-하", label: "테이블 2번" },
];
