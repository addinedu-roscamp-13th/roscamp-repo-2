          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-0.5 flex items-center gap-1.5 text-[11px]">
                ...
                <span
                  className={b.inStock ? "text-emerald-600" : "text-rose-500"}
                >
                  {b.inStock ? "✅" : "⛔"}
                </span>
              </div>
...
function ChatPage() {
  ...
  const send = async (text: string) => {
      ...
      setMessages((m) =>
        m.map((msg) =>
          msg.id === pendingId
            ? {
                ...msg,
                text: `🤖 로봇 명령을 실행 중입니다... (${directCmd.robot_type} - ${directCmd.action})`,
                pending: true,
              }
            : msg,
        ),
        ...
        setMessages((m) =>
          m.map((msg) =>
            msg.id === pendingId
              ? {
                  ...msg,
                  text: "❌ 백엔드 서버와의 연결에 실패하여 로봇을 제어할 수 없습니다.",
                  pending: false,
                }
              : msg,
          ),
    ...
    try {
      ...
      const showMap =
        /(화장실|restroom|toilet|지도|map|洗手|지도|map|洗手|지도|map|洗手|지도|map|지도|map|洗手|地图|vệ sinh|bản đồ)/i.test(
          text,
        );
      ...
      if (isRecommendIntent(text)) {
        books = await fetchRecommendedBooks({
      ...
      if (robotCmdJson) {
        ...
        setMessages((m) =>
          m.map((msg) =>
            msg.id === pendingId
              ? { ...msg, showMap, books, pending: false }
              : msg,
          ),
        );
