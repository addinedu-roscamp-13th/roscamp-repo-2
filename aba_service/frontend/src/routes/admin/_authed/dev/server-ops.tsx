      ...
      <div className="flex flex-col gap-4">
        <div>
          ...
          <p className="mt-1 text-[13px] text-slate-500">
            PM2로 구동되는 FastAPI 백엔드와 nginx의 실행·재시작·재배포 방법입니다.
          </p>
        </div>

        <Section title="백엔드 프로세스 (PM2)">
          <div className="flex flex-col gap-4">
            <div className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-[13px] text-blue-800">
              백엔드는 PM2 프로세스 <code className="font-mono font-semibold">aba-service-backend</code> (:3001) 로 구동되며, <code className="font-mono">pm2-ane.service</code> + <code className="font-mono">pm2 save</code> 로 부팅 시 자동 시작되고, 크래시 시 자동 재시작됩니다.
            </div>
            <div className="grid gap-2 md:grid-cols-2">
              <div className="flex flex-col gap-2">
                ...
                <CodeBlock code="pm2 status aba-service-backend" />
              </div>
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2 text-[13px] text-slate-600">
                  <RotateCcw className="h-4 w-4 text-slate-400" /> 재시작
                </div>
                <CodeBlock code="pm2 restart aba-service-backend" />
              </div>
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2 text-[13px] text-slate-600">
                  <Square className="h-4 w-4 text-slate-400" /> 중지
                </div>
                <CodeBlock code="pm2 stop aba-service-backend" />
              </div>
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2 text-[13px] text-slate-600">
                  <Search className="h-4 w-4 text-slate-400" /> 실시간 로그
                </div>
                <CodeBlock code="pm2 logs aba-service-backend" />
              </div>
            </div>
          </div>
        </Section>
        ...
        <Section title="코드 변경 후 재배포">
          <div className="flex flex-col gap-5">
            <Step num={1} label="백엔드 변경 시 — PM2 재시작">
              <CodeBlock code="pm2 restart aba-service-backend" />
            </Step>
            <Step num={2} label="프론트엔드 변경 시 — 빌드 후 nginx 리로드">
              <CodeBlock code={`cd /home/roscamp-repo-2/aba_service/frontend
        ...
        <Section title="nginx (정적 서빙 + 프록시)">
          <div className="flex flex-col gap-4">
            <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-[13px] text-amber-800">
              ...
              {" "}— <code className="font-mono">/</code>→dist, <code className="font-mono">/api</code>→:3001, <code className="font-mono">/ollama</code>→:11434
            </div>
            <div className="flex flex-col gap-2">
...
sudo systemctl reload nginx`} />
        ...
        <Section title="서버 정보">
          <div className="flex flex-col">
            <InfoRow label="프론트 포트" value="3000 (nginx, ngrok 연결)" />
            <InfoRow label="백엔드 포트" value="3001 (FastAPI / uvicorn)" />
            <InfoRow label="PM2 프로세스" value="aba-service-backend" />
