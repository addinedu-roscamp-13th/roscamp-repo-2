...
class LineConfig(BaseModel):
    ...
    # ── 직진/조향 ──
    straight_only: bool = True                        # true = 라인으로 몸을 돌리지 않고 직진만 유지
    # IR 3센서 조향(straight_only=False): 테이프가 좌(우) 센서에 걸리면 좌(우)로 보정.
    # 각속도 + 는 좌회전(오른바퀴 가속) — 테이프가 좌측 센서에 보이면 +.
    steer_soft: float = Field(0.05, ge=0.0, le=0.3)   # 중앙+가장자리 함께 보일 때(살짝 보정)
    steer_hard: float = Field(0.10, ge=0.0, le=0.4)   # 가장자리 센서에만 보일 때(강한 보정)
    steer_lin_scale: float = Field(0.6, ge=0.2, le=1.0)  # 조향 중 전진 감속 비율
    ...
    target_wall_cm: float = Field(8.0, ge=2.0, le=80.0)   # 이 거리 도달 시 주차 완료
    hard_stop_wall_cm: float = Field(8.0, ge=2.0, le=80.0)  # 안전 하한: 목표보다 늦어지면 여기서 강제 정지
    slow_wall_cm: float = Field(22.0, ge=5.0, le=150.0)   # 이 안쪽부터 감속
    ...
    line_end_done_wall_cm: float = Field(15.0, ge=2.0, le=80.0)  # 라인 끝났고 벽이 이 안이면 완료로 간주
...
async def _read_wall_cm() -> float | None:
    try:
        ...
        if d < 0.5:
            ...
            # 버리면 가까울수록 벽이 '안 보여' 그대로 박치기 — 초근접(2cm)으로 클램프.
...
async def _line_loop(cfg: LineConfig) -> None:
    ...
    def _boost_pair(l: int, r: int) -> tuple[int, int]:
    ...
    try:
        ...
        while True:
            ...
            if wall is not None:
                ...
                wall_valid_t = time.time()
            elif cfg.use_wall_sensor and time.time() - wall_valid_t > 2.5:
                # 벽 거리가 유일한 정지 조건인데 초음파가 계속 무효면 눈감고 전진하는 셈 —
                # 평활값이 얼어붙은 채 벽을 지나친 실사고(2026-07-07, 32.9cm 동결) 재발 방지.
                await _motor_send(0, 0)
                _state.update(phase="error", message="초음파 응답 없음 2.5초 지속 — 안전 정지", telemetry={"left": 0, "right": 0, "wall_cm": round(wall_f, 1) if wall_f is not None else None, **det})
                return
            ...
            prev_wall = wall
            if wall_hit and (near_prev_wall or wall_hits >= 2):
                await _motor_send(0, 0)
                if cfg.rear_finish:
            ...
            if wall_f is not None and wall_f < cfg.slow_wall_cm:
                frac = max(0.0, (wall_f - cfg.target_wall_cm) / (cfg.slow_wall_cm - cfg.target_wall_cm))
                linear *= max(0.3, frac)

            left, right = _vel_to_speeds(linear, angular)
            left, right = _boost_pair(left, right)
...
async def line_detect(ir_white_max: int = 800):
    ir = await _read_ir()
    wall = await _read_wall_cm()
    on_tape = _ir_on_tape(ir, ir_white_max)
    return {
        "found": any(on_tape.values()),
        "ir": ir,
        "on_tape": on_tape,
