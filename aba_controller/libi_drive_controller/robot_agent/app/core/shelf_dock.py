...
#:
#: ## [2026-08-07] 0.09 → 0.088 — 사용자 지시로 2mm 좁힘
#:
#: 실여유 3cm → **2.8cm**. 팔 도달 거리를 벌기 위한 조정이다.
#:
#: ⚠️ 이 값은 **제자리 회전의 안전 여유이기도 하다.** 도킹을 마치면 옆구리가 서가를
#: 보고, 빠져나갈 때 90° 를 제자리에서 돈다 — 그때 몸 끝단이 이 거리 안으로 들어오면
#: 닿는다. 2026-08-07 실기에서 꽁무늬가 닿아 회전 방향을 뒤집었다
#: (`shelf/geometry.py` `retreat_moves`). 여기를 더 줄이면 그 여유를 같이 깎는 것이니,
#: 다음에 줄일 때는 **회전 중 몸 끝단이 쓸고 가는 반경을 실측하고** 줄인다.
CLEARANCE_M = 0.088
