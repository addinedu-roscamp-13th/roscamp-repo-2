...
## ⚠️ [2026-07-30] 이제 BT 가 이 액션을 **부른다** — 그런데 부르려던 상대는 여기가 아니다

복귀 단계가 재편되면서 ④단계(`ArucoApproach`)가 `/fleet_cmd{action}` 을 낸다
(`libi_modes/main.py` 의 `return_aruco` 드라이버, 액션 이름은 `dock_action` 파라미터).

**기본값은 `"dock"` 이 아니라 `"aruco_dock"` 이다** — 즉 기본 설정에서 그 명령은 여기로
안 온다. 일부러 그렇게 뒀다: `"dock"` 이면 외부 노드가 없어도 아래 라인 트레이싱이
실제로 로봇을 움직이고 성공까지 답한다. 뒷캠 ArUco 인 줄 알고 있는데 다른 알고리즘이
도는, 가장 나쁜 종류의 조용한 실패다. 모르는 액션이면 `400 알 수 없는 action` 이 즉시
돌아와 BT 가 재시도 끝에 ERROR 로 간다 — 시끄럽게 실패한다.

이걸(라인 트레이싱) 쓰고 싶으면 `-p dock_action:=dock` 로 되돌린다.

⚠️ **어느 이름을 쓰든 답하는 쪽은 하나여야 한다.** 둘이 답하면 FMS/BT 는 **먼저 온
결과**로 다음 단계를 시작한다 — 로봇이 아직 접근 중인데 3cm 개루프 후진이 겹친다.
팔에서 이미 밟은 함정이다(CLAUDE.md `LIBI_ARM_VIA_BT`).

⚠️ **취소 계약이 아직 없다.** BT 가 timeout(180s) 뒤 내는 `stop` 은 fleet_link 에서
`ros_bridge.cancel_nav()` 로 가서 **nav2 만** 끊는다 — 여기서 도는 `_park_loop` 도,
외부 노드의 ArUco 접근도 안 멈춘다. 그 상태로 `AbsorbFailure` 가 재시도하면 이전
접근과 새 접근이 겹친다. 외부 노드를 붙일 때 action 별 cancel + ack 를 같이 정할 것.

`/is_docked` 는 여전히 `dock_confirm.py` 가 위치(반경 0.12m)로 판정한다. 다만 BT 의
마지막 단계는 [2026-07-30] 부터 그 신호를 **안 본다**(시간 대기로 바뀌었다).
