...
class LidarDockConfig:
    ...
    #     종료점이 정지 목표 바로 옆(6.5cm)까지 붙어 실기에서 다시 벌려
    #     달라는 요청. y 를 확실히 맞추고 싶어서 1cm 더(사용자 요청,
    #     6차 yaw 우선순위 램프의 "가까움" 기준점이기도 하다 — 늘리면
    #     그 램프가 완전히 다 켜질 거리도 그만큼 더 여유가 생긴다).
    #     stop_m(6.5cm) + 이 값 = 13.5cm 지점에서 조향을 끈다.
    waypoint_margin_m: float = 0.07
    # ── 속도 ──────────────────────────────────────────────────────────────
    ...
    #   ⚠️ [2026-08-04 실측] `_angular()` 는 `lin_mag==0` 이면 조향도 강제로 0 이다
    #   (approach.py 참고 — 정지 중 조향 명령은 정지마찰 못 이겨 잡음이라서). 그런데
    #   펄스 구간(15cm 안)은 밀고-쉬고를 반복하는데, 쉬는 동안은 그래서 조향이 전혀
    #   안 걸린다. 0.30초였을 때 밀기(≥0.10초):쉬기 비가 최대 1:3 이라, 이 구간에서
    #   실제로 조향이 걸리는 시간이 전체의 25% 밖에 안 됐다("게인을 올려도 덜 간다"는
    #   실기 관찰과 일치). 쉬는 시간을 줄여 조향 걸리는 비중을 늘린다.
    pulse_pause_s: float = 0.15
    # ── 조향 ──────────────────────────────────────────────────────────────
