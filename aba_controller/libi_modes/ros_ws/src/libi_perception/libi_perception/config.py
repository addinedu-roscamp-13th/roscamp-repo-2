...
#   [2026-08-01] **28 → 69. 실측 화면으로 사용자가 직접 잡은 경계다.**
#
#   패널 스크린샷 세 장에서 bbox 를 재고 640 기준으로 환산했다(화면 표시영역
#   564×427, 원본 320×240 이므로 화면픽셀 × 0.567 × k(=2.0)):
#
#       "이 정도면 전진"  √area ≈ **214**
#       "이 정도면 후진"  √area ≈ **352**
#
#   가운데가 (214+352)/2 = 283 으로 지금 `TARGET_SIZE=280` 과 사실상 같다 —
#   목표는 그대로 두고 폭만 넓히면 그 경계가 된다: 280 ± 69 = **211 ~ 349**.
#   비율로는 목표의 10% → **25%**, 거리로는 1.0m 에서 ±10cm → **±25cm** 다.
#
#   좁던 10% 는 사람이 조금만 움직여도 로봇이 따라 붙어 계속 깨작이게 했다.
DIST_DEADZONE = 69.0          # sqrt(area) px @640 = 목표의 25% — 이 안이면 linear_x = 0
...
# [2026-08-01] **45 → 106.67. 화면의 3등분 가운데 칸과 같게 맞춘 것이다.**
#
#   서버가 영상에 굽는 가이드선은 `w//3`, `2*w//3` 다(perception_server.draw_overlay).
#   즉 가운데 칸은 중심에서 **±w/6**, 640 기준으로 **±106.67px** 다. 그런데 데드존이
#   45px 라 가운데 칸의 **42%** 만 정지 구간이었다 — 사람이 화면상 명백히 가운데
#   칸 안에 있는데도 로봇이 계속 좌우로 깨작였다. "가운데 있는데 왜 움직여"가 이것.
#
#   눈에 보이는 선과 제어의 경계가 다르면 화면이 거짓말을 한다. 같게 둔다.
#
#   ⚠️ 이 값을 넓히려면 `pid.py` 의 방위 데드존이 **오차를 빼내야** 한다. 예전처럼
#      0 으로 죽이면 경계 바로 밖에서 `KP_ANGLE × 106.67 = 0.107 rad/s` 가 통째로
#      튀어나온다(45일 때는 0.045 라 덜 티났다). 그래서 같은 커밋에서 거리축과 같은
#      방식으로 고쳤다.
ANGLE_DEADZONE = 106.67       # px @640 — 폴백. 아래 FRAC 이 있으면 그쪽이 이긴다
# [2026-08-01] **고정 픽셀 대신 화면 비율.**
#   가이드선이 `w//3`·`2w//3` 이라 가운데 칸은 언제나 중심 ±w/6 이다. 비율로 두면
#   해상도가 320 이든 640 이든 **눈에 보이는 그 칸**이 그대로 정지 구간이 된다.
#   고정 픽셀은 해상도가 바뀔 때마다 화면과 조용히 어긋난다. 0 이면 구간을 끈다.
ANGLE_DEADZONE_FRAC = 1.0 / 6.0
# 멈춘 뒤 **다시 돌기 시작하는** 문턱 배수. 1.0 이면 히스테리시스 없음.
#   문턱이 하나면 칸 경계에서 켜졌다 꺼졌다 하며 좌우로 깨작인다("도리도리").
#   1.3 = 가운데 칸을 30% 더 벗어나야 다시 돈다. 대신 한 번 돌기 시작하면 오차를
#   통째로 써서 **확실히** 돈다(경계 근처에서 힘없이 못 도는 문제도 같이 없어진다).
#   ⚠️ 키우면 더 둔해지고 줄이면 더 예민해진다. 실측으로 조정할 값이다.
ANGLE_RESUME_RATIO = 1.3
