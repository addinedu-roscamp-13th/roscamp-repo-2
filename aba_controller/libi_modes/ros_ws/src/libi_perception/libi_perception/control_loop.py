...
class ControlLoop:
    ...
    def __init__(self, get_detection, get_scan, publish, cfg, now=time.monotonic,
        ...
        #:
        #: ⚠️ [2026-08-07] **가운데여도 돈다.** 예전엔 가운데면 peek 를 껐는데
        #:    (2026-08-02 지시) 실기에서 대상을 정면에서 치우는 경우가 가장 흔해
        #:    peek 이 거의 안 걸렸다. 이제 방향만 정하고 켜고 끄지는 않는다 —
        #:    가운데면 `lkd` 가 로봇이 돌던 방향으로 떨어진다(`_build_search`).
        #:
        ...
        #: 이번 유실에서 **코스팅 박스(주황)를 실제로 봤나.** 실검출이 돌아오면 되감는다.
        #: ⚠️ [2026-08-07] **peek 조건에서 빠졌다** — 이제 로그 래치로만 쓴다.
        #:    근거와 경위는 `_build_search` 의 `do_peek` 주석.
    ...
    def _filtered_detection(self):
        ...
        **반드시 이 함수를 통해서만** 검출을 봐야 한다 — 원본 `self.get_detection`
        ...
        시간으로 재는 이유: 제어 루프(20Hz)와 검출(15fps)의 주기가 달라 **같은 예측
    ...
    def _build_search(self):
        ...
        # ⚠️ [2026-08-07] **놓치면 언제나 peek 한다 — 조건을 다 뺐다.**
        #
        #   사용자 지시 2026-08-07: "그냥 나는 인형이 사라진 곳으로 peek 동작을 시키고
        #   싶어". 소실 = peek 이고, 방향은 위 `lkd` 하나가 정한다.
        #
        #   빠진 조건 둘과 그 경위:
        #
        #   ① `and self._coast_seen`(주황 코스팅 박스를 봤나) — 2026-08-06 에 넣었다가
        #      뺐다. 근거는 "주황 박스가 없으면 사라진 방향에 대한 근거가 없다" 였는데
        #      전제가 틀렸다 — 방향의 근거는 `_last_side` 고 그건 **실검출 프레임에서도
        #      갱신된다**(아래 `tick()`). 코스팅은 "얼마나 오래 붙들었나" 이지 "어느
        #      쪽이었나" 가 아니다. 게다가 코스팅이 아예 안 도는 경로가 흔해
        #      (`may_coast` 가 막는 자세·방향, 그리고 가림 없이 화면 밖으로 나가는 경우)
        #      **peek 을 통째로 끄는 스위치**로 작동했다.
        #
        #   ② `self._last_side in (-1, 1)`(가운데 칸이면 끔) — 2026-08-02 지시
        #      "가운데에서 사라지면 peek 가 없어도 될 것 같다" 로 넣었던 것이다.
        #      실기에서 **대상을 정면에서 치우는 경우가 가장 흔해서**(인형을 그냥 빼면
        #      마지막 칸이 가운데다) peek 이 거의 안 걸렸다. 2026-08-07 지시로 뒤집는다.
        #
        #   ⚠️ 가운데에서 사라지면 방향 근거가 약한 것은 **여전히 사실이다.** 그때
        #      `lkd` 는 로봇이 돌던 방향(`tracker.last_direction`, 없으면 +1)으로
        #      떨어진다 — 추측이지만 "안 도는 것" 보다 낫다는 것이 이번 결정이다.
        #      되돌리려면 이 줄만 `self._last_side in (-1, 1)` 로 바꾼다.
        #
        #   `_coast_seen` 은 그대로 남는다 — 코스팅 시작/끝 로그의 래치로만 쓴다.
        do_peek = True
