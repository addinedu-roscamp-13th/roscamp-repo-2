...
#
# ⚠️ [2026-08-06] **`Calibrating` 을 뺐다 — 남은 것은 `Lying` 하나다.**
#
#   넣었던 이유는 "기준을 재는 중이라 자세 판정 자체를 못 믿는다" 였다. 그런데
#   **못 믿는 것과 막는 것은 다른 결정**이고, 같은 "모른다"인 `Unknown` 은 이미
#   허용하고 있었다. 일관되지 않았다.
#
#   막으면 무슨 일이 생기나 (사용자 보고 2026-08-06: "pose 켜면 주황 박스가 안 보인다"):
#     · 등록 직후 캘리브 구간은 **정상 조건에서도 60프레임 ≈ 3.5초** 다. 그 안에
#       놓치면 코스팅이 통째로 꺼진다.
#     · 골격이 잘 안 잡히면(역광·측면·먼 거리) 표본이 안 모여 **더 길어진다.**
#     · `--pose` 를 끄면 자세가 None 이라 안 걸린다 → "골격 없이는 잘 됐다".
#
#   빼도 안전한 이유 — **로봇은 어차피 안 움직인다.** `PostureGate` 가 `Calibrating`
#   을 즉시정지로 보고, 코스팅은 `motion_ok` 를 **마지막 실제 판정에서 물려받는다**
#   (`pipeline.get_latest` 의 그 주석). 즉 코스팅을 허용해도 `motion_ok=False` 라
#   전진은 0 이다. 바뀌는 것은 **화면에 주황 박스가 보이고, 회복 탐색이 사라진
#   방향을 알게 되는 것**뿐이다. 위험은 안 늘고 정보만 는다.
#
